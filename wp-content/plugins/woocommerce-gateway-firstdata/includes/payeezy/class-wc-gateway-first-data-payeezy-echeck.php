...
class WC_Gateway_First_Data_Payeezy_eCheck extends WC_Gateway_First_Data_Payeezy {
	...
	protected function validate_check_fields( $is_valid ) {
		...
		$id_number    = SV_WC_Helper::get_post( 'wc-' . $this->get_id_dasherized() . '-customer-id-number' );

		// check number is always required
		if ( empty( $check_number ) ) {

			SV_WC_Helper::wc_add_notice( esc_html__( 'Check Number is missing', 'woocommerce-gateway-firstdata' ), 'error' );
			$is_valid = false;
		}
		...
		if ( empty( $id_number ) ) {
			...
			SV_WC_Helper::wc_add_notice( esc_html__( 'ID number is missing', 'woocommerce-gateway-firstdata' ), 'error' );
			$is_valid = false;

		} elseif ( ! preg_match( '/^[a-zA-Z0-9 -]+$/', $id_number ) ) {

			SV_WC_Helper::wc_add_notice( esc_html__( 'ID number is invalid', 'woocommerce-gateway-firstdata' ), 'error' );
			$is_valid = false;
		}
	...
	public function get_order( $order_id ) {
		...
		$order->payment->customer_id_type   = SV_WC_Helper::get_post( 'wc-' . $this->get_id_dasherized() . '-customer-id-type' );
