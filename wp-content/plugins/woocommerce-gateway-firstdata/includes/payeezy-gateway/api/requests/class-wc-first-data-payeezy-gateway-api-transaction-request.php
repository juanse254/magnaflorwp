...
class WC_First_Data_Payeezy_Gateway_API_Transaction_Request extends WC_First_Data_Payeezy_Gateway_API_Request {
	...
	protected function create_transaction( $type ) {
		...
		$this->request_data = array(
			...
			'reference_3'      => SV_WC_Order_Compatibility::get_prop( $this->get_order(), 'id' ),
			'client_ip'        => SV_WC_Order_Compatibility::get_prop( $this->get_order(), 'customer_ip_address' ),
	...
	protected function set_address_data() {
		...
		$this->request_data = array_merge( $this->request_data, array(
			'zip_code' => SV_WC_Order_Compatibility::get_prop( $this->get_order(), 'billing_postcode' ),
			'address' => array(
				'address1'     => SV_WC_Helper::str_truncate( SV_WC_Order_Compatibility::get_prop( $this->get_order(), 'billing_address_1' ), 30 ),
				'address2'     => SV_WC_Helper::str_truncate( SV_WC_Order_Compatibility::get_prop( $this->get_order(), 'billing_address_2' ), 28 ),
				'city'         => SV_WC_Helper::str_truncate( SV_WC_Order_Compatibility::get_prop( $this->get_order(), 'billing_city' ), 20 ),
				'state'        => SV_WC_Helper::str_truncate( SV_WC_Order_Compatibility::get_prop( $this->get_order(), 'billing_state' ), 2 ),
				'zip'          => SV_WC_Helper::str_truncate( SV_WC_Order_Compatibility::get_prop( $this->get_order(), 'billing_postcode' ), 10 ),
				'country_code' => SV_WC_Helper::str_truncate( SV_WC_Order_Compatibility::get_prop( $this->get_order(), 'billing_country' ), 2 ),
				'phone_number' => SV_WC_Helper::str_truncate( preg_replace( '/\D/', '', SV_WC_Order_Compatibility::get_prop( $this->get_order(), 'billing_phone' ) ), 14 ),
				'phone_type'   => 'D',
			),
		) );
	}
	...
	protected function get_reference_no() {
		...
		if ( 'US' === SV_WC_Order_Compatibility::get_prop( $this->get_order(), 'billing_country' ) ) {
