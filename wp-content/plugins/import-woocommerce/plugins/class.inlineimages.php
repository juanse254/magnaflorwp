...
class WoocomImporter_inlineImages {
	...
	public function process_multi_images($data_array, $helperObj, $currentLimit, $impObj, $import_image_method, $imgLoc, $sampleURL) {
		...
		foreach($res as $key => $image_url) {
			foreach($image_url as $img_path) {
				...
				$imagelist = csvscanDirectories($inlineimageDirpath);
				if(!$imagelist) {
	...
	$post_content = str_replace("\n", "<br />", $post_content);
		...
		foreach($inlineimg_shortcodes as $shortkey => $shortcode){
			...
			$eventKey = sanitize_text_field($_POST['postdata']['uploadedFile']);
                        ...
                        $imagelist = csvscanDirectories($inlineimageDirpath);
                        if(!$imagelist) {
				$noimage = Woocom_CONST_CSV_IMP_DIR . "images/noimage.png";
				$oldWord = $shortcode;
...
function csvscanDirectories($rootDir, $allData=array()) {
         ...
         foreach($dirContent as $key => $content) {
                ...
                if(!in_array($content, $invisibleFileNames)) {
                        ...
                        if(is_file($path) && is_readable($path)) {
                                ...
                                $allData = csvscanDirectories($path, $allData);
                        }
                }
        }
        return $allData;
