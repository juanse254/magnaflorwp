...
self.stickyHeaderMenu();self.stickyVcexNavbar();self.headerOverlayOffset();self.barterReveal();self.fixedFooter();self.titleBreadcrumbsFix();if($.fn.infinitescroll!==undefined&&$('div.infinite-scroll-nav').length){self.infiniteScrollInit()}
...
if(self.config.$body.hasClass('wpex-has-fixed-barter')){self.config.hasFixedFooter=!0}
self.config.$barterReveal=$('.barter-reveal-visible');if(self.config.$barterReveal.length&&self.config.$siteWrap&&self.config.$siteMain){self.config.$hasFooterReveal=!0}
...
if($windowPos>=$divPos&&$windowPos<($divPos+$divHeight)){$higlight_link.addClass('active');$higlight_link.parent('li').addClass('current-menu-item')}else{$higlight_link.removeClass('active');$higlight_link.parent('li').removeClass('current-menu-item')}}},equalHeights:function($context){if($.fn.wpexEqualHeights!==undefined){$('.match-height-grid',$context).wpexEqualHeights({children:'.match-height-content'});$('.match-height-row',$context).wpexEqualHeights({children:'.match-height-content'});$('.vcex-feature-box-match-height',$context).wpexEqualHeights({children:'.vcex-match-height'});$('.blog-equal-heights',$context).wpexEqualHeights({children:'.blog-entry-inner'});$('.related-posts',$context).wpexEqualHeights({children:'.related-post-content'});$('.wpex-vc-row-columns-match-height',$context).wpexEqualHeights({children:'.vc_column-inner'});$('.wpex-vc-columns-wrap',$context).wpexEqualHeights({children:'.equal-height-column'});$('.wpex-vc-columns-wrap',$context).wpexEqualHeights({children:'.equal-height-content'})}},barterReveal:function(){var self=this;if(!self.config.$hasFooterReveal){return}
var $barterReveal=self.config.$barterReveal;function showHide(){if(self.config.viewportWidth<960){if($barterReveal.hasClass('barter-reveal')){$barterReveal.toggleClass('barter-reveal barter-reveal-visible');self.config.$siteWrap.css('margin-bottom','')}
return}
var $hideFooter=!1,$barterRevealHeight=$barterReveal.outerHeight(),windowHeight=self.config.windowHeight,$heightCheck=0;if($barterReveal.hasClass('barter-reveal')){$heightCheck=self.config.$siteWrap.outerHeight()+self.config.localScrollOffset}else{$heightCheck=self.config.$siteWrap.outerHeight()+self.config.localScrollOffset-$barterRevealHeight}
if((windowHeight>$barterRevealHeight)&&($heightCheck>windowHeight)){$hideFooter=!0}
if($hideFooter&&$barterReveal.hasClass('barter-reveal-visible')){self.config.$siteWrap.css({'margin-bottom':$barterRevealHeight});$barterReveal.removeClass('barter-reveal-visible');$barterReveal.addClass('barter-reveal')}
if(!$hideFooter&&$barterReveal.hasClass('barter-reveal')){self.config.$siteWrap.css('margin-bottom','');$barterReveal.removeClass('barter-reveal');$barterReveal.removeClass('wpex-visible');$barterReveal.addClass('barter-reveal-visible')}}
function reveal(){if($barterReveal.hasClass('barter-reveal')){if(self.scrolledToBottom(self.config.$siteMain)){$barterReveal.addClass('wpex-visible')}else{$barterReveal.removeClass('wpex-visible')}}}
