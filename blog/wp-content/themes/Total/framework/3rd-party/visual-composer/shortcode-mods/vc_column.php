...
if ( ! class_exists( 'VCEX_VC_Column_Config' ) ) {
	...
	class VCEX_VC_Column_Config {
		...
		public function __construct() {
			...
			add_filter( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, array( 'VCEX_VC_Column_Config', 'shortcode_classes' ), 10, 3 );
		}
...
/**
 * Adds extra attibutes to the vc_column inner element
 *
 * @since 4.0
 */
function wpex_vc_column_inner_extra_attributes( $atts ) {
	...
	if ( ! empty( $atts['min_height'] ) ) {
		$inline_style .= 'min-height:' . $atts['min_height'] . ';';
	}
