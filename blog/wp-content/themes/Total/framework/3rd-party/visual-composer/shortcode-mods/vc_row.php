...
if ( ! class_exists( 'VCEX_VC_Row_Config' ) ) {
	...
	class VCEX_VC_Row_Config {
		...
		public function __construct() {
			...
			add_action( 'wpex_vc_modify_params', array( 'VCEX_VC_Row_Config', 'update_params' ) );
		...
		public static function add_params( $params ) {
			...
			$add_params[] = array(
				...
				'description' => __( 'Check this box to make all columns inside this row full-width for tablets.', 'total' ),
			);
			$add_params[] = array(
				'type' => 'dropdown',
			...
			$add_params[] = array(
				...
				'param_name' => 'video_bg',
				'description' => __( 'Check this box to enable the options for a self hosted video background.', 'total' ),
				'value' => array(
					__( 'None', 'total' ) => '',
		...
		public static function shortcode_classes( $class_string, $tag, $atts ) {
			...
			/* Equal height
			// @deprecated in 4.0
			if ( ! empty( $atts['match_column_height'] ) && 'yes' == $atts['match_column_height'] ) {
				$add_classes[] = 'wpex-vc-row-columns-match-height';
			} elseif ( vc_is_inline() ) {
				$add_classes[] = 'wpex-vc-row-reset-columns-match-height';
			}*/

			// Centered row
			if ( ! empty( $atts['center_row'] ) && 'yes' == $atts['center_row'] ) {
			...
			// Tablet
			if ( ! empty( $atts['tablet_fullwidth_cols'] ) && 'yes' == $atts['tablet_fullwidth_cols'] ) {
				$add_classes[] = 'tablet-fullwidth-columns';
			}

			// Right hand columns
			if ( ! empty( $atts['columns_right'] ) && 'yes' == $atts['columns_right'] ) {
			...
			if ( ! empty( $atts['wpex_self_hosted_video_bg'] ) ) {
				$add_classes[] = 'wpex-video-bg-wrap';
				$add_classes['wpex-vc_row-has-fill'] = 'wpex-vc_row-has-fill';
			}
...
function wpex_hook_vc_row_top( $atts = '' ) {
	return apply_filters( 'wpex_hook_vc_row_top', '', $atts ); // Must be filter since do_action returns content
}
...
function wpex_hook_vc_row_bottom( $atts = '' ) {
	return apply_filters( 'wpex_hook_vc_row_bottom', '', $atts ); // Must be filter since do_action returns content
}