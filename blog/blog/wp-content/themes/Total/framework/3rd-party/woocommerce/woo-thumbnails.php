...
if ( ! class_exists( 'WPEX_WooCommerce_Thumbnails' ) ) {
	...
	class WPEX_WooCommerce_Thumbnails {
		...
		public function __construct() {
			...
			// Breaks because the new image resize function calls the images via wp_get_attachment_image_src
			// @todo support image resizer inside hook
		...
		public static function attachment_image_src( $image, $attachment_id, $size, $icon ) {
			...
			// Shop thumbnail
			if ( 'shop_thumbnail' == $size ) {

				$generate_image = wpex_get_post_thumbnail_src( array(
					'attachment'               => $attachment_id,
					'size'                     => 'shop_single_thumbnail',
					'get_attachment_image_src' => false, // IMPORTANT !!
				) );
