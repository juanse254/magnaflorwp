...
// Add main image to attachments list
...
<div class="images clr">
	...
	if ( $attachments && $attachements_count > 1 && $show_slider ) :
		...
		<div class="wpex-slider pro-slider lightbox-group"<?php echo $data_atributes_html; ?>>
	...
	elseif ( has_post_thumbnail() || isset( $attachments[0] ) ) :
		...
		if ( $product->has_child() ) {
			echo apply_filters( 'woocommerce_single_product_image_thumbnail_html', sprintf( '<div class="woocommerce-main-image">%s</div>', $image ), $post->ID );
		} else {
			echo apply_filters( 'woocommerce_single_product_image_thumbnail_html', sprintf( '<a href="%s" itemprop="image" class="woocommerce-main-image wpex-lightbox" title="%s" data-show_title="false">%s</a>', $image_link, $image_title, $image ), $post->ID );
		}

		// Display variation thumbnails
		if ( $product->has_child() || ! $show_slider ) {

			$wrap_classes = 'product-variation-thumbs thumbnails clr';
			if ( $attachements_count <= 1 ) {
				$lightbox_class = 'zoom wpex-lightbox';
			} else {
				$wrap_classes .= ' lightbox-group';
				$lightbox_class = 'zoom wpex-lightbox-group-item';
			} ?>
			...
			<div class="<?php echo esc_attr( $wrap_classes ); ?>">
				...
				foreach ( $attachments as $attachment ) :
					...
					<?php if ( $thumbnail ) : ?>
						...
						<a href="<?php echo wpex_get_lightbox_image( $attachment ); ?>" title="<?php echo esc_attr( $attachment_alt ); ?>" data-title="<?php echo esc_attr( $attachment_alt ); ?>" data-type="image" class="<?php echo esc_attr( $lightbox_class ); ?>" data-show_title="false"><?php echo $thumbnail; ?></a>
